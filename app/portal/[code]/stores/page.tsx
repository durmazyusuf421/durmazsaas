...
import { 
  ...
  ShoppingCart, Rocket, UserCircle, ArrowRight, Barcode
} from 'lucide-react';
...
export default function StoreSelectionPage() {
  ...
  if (loading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-[#F4F7FE] gap-4">
      ...
      <p className="text-[#1B2559] font-bold uppercase tracking-widest text-xs">Mağazalar Hazırlanıyor...</p>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#F4F7FE] flex font-sans">
      ...
      {/* SOL MENÜ (Sidebar - POS ile Birebir Aynı) */}
      ...
      <main className="flex-1 lg:ml-72 p-6 md:p-12">
        <div className="max-w-5xl mx-auto space-y-10">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-8 rounded-[40px] shadow-sm border border-white gap-6">
            <div>
              <h1 className="text-3xl font-black text-[#1B2559] uppercase tracking-tighter leading-none">Sipariş Ver</h1>
              <p className="text-gray-400 font-bold text-[10px] uppercase mt-2 tracking-widest">Alışveriş yapacağınız toptancıyı seçin</p>
            </div>
            <div className="flex items-center gap-4 bg-gray-50 px-6 py-3 rounded-3xl border border-gray-100">
              ...
              <span className="font-black text-[#1B2559] uppercase text-xs">{profile?.full_name || 'Müşteri'}</span>
            </div>
          </div>
          ...
          <div className="space-y-6">
             {linkedBusinesses.length === 0 ? (
                <div className="bg-white p-20 rounded-[50px] border-2 border-dashed border-gray-100 text-center shadow-sm">
                    ...
                    <h3 className="text-2xl font-black text-[#1B2559] uppercase tracking-tighter">İş Ortığı Bulunamadı</h3>
                    <p className="text-gray-400 font-bold mt-2 text-sm">Sipariş verebileceğiniz aktif bir toptancı bağlantısı yok.</p>
                </div>
             ) : (
                    ...
                    {linkedBusinesses.map((b, index) => (
                      <div key={index} className="bg-white p-8 rounded-[40px] shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all border border-gray-50 flex flex-col md:flex-row items-center justify-between gap-8 group">
                        <div className="flex items-center gap-6 w-full md:w-auto">
                            <div className="w-20 h-20 bg-blue-50 text-blue-500 rounded-3xl flex items-center justify-center group-hover:bg-blue-600 group-hover:text-white transition-all">
                                <Store size={36}/>
                            </div>
                            <div>
                                <h3 className="text-2xl font-black text-[#1B2559] uppercase mb-1 tracking-tighter">{b.companies?.name}</h3>
                                <div className="flex items-center gap-2 text-[10px] font-bold text-green-500 uppercase tracking-widest">
                                    <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span> Siparişe Açık
                                </div>
                            </div>
                        </div>
                        ...
                        <Link 
                          ...
                          className="w-full md:w-auto px-10 py-5 bg-[#3063E9] text-white rounded-3xl font-black uppercase tracking-widest shadow-xl shadow-blue-500/30 hover:bg-blue-700 transition-all flex items-center justify-center gap-3 text-sm"
                        >
                          <ShoppingCart size={20}/> Mağazaya Gir <ArrowRight size={18} />
                        </Link>
                      </div>
                    ))}
                </div>
